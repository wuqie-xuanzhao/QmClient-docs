...
> 当前入口是 [自适应AntiPing状态.html](../../规划/自适应AntiPing状态.html)。
>
> 本文是旧调研/旧方案，只保留历史上下文。当前代码已经有 DDNet AntiPing、TClient improved AntiPing smoothing、预测 margin 自动补偿、QmMonitoring 网络指标和 TClient status bar 网络显示。实现前请先读 `自适应AntiPing状态.html`；不要直接按本文新增 `cl_antiping_adaptive` 或自动改写预测参数。
